...
void Camera::draw(Vector2 mouseDelta)
{
	...
	if (lookTarget != NULL )
	{
		...
		gluLookAt(position.x, position.y, position.z,
			lookTarget->getPosition().x / lookTarget->getPositionScale(),
			lookTarget->getPosition().y / lookTarget->getPositionScale(),
			lookTarget->getPosition().z / lookTarget->getPositionScale(),
			0.0f, 1.0f, 0.0f);
