...
bool IntersectionTests::sphereAndSphere(const CollisionSphere &one, const CollisionSphere &two)
{
	...
	return midline.magnitudeSquared() < pow((one.radius + two.radius), 2);
}
...
bool IntersectionTests::boxAndBox(const CollisionBox &one, const CollisionBox &two)
{
	...
	return (
		...
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(0), two.getAxis(0)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(0), two.getAxis(1)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(0), two.getAxis(2)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(1), two.getAxis(0)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(1), two.getAxis(1)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(1), two.getAxis(2)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(2), two.getAxis(0)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(2), two.getAxis(1)), toCenter) &&
		overlapOnAxis(one, two, Vector3::crossProduct(one.getAxis(2), two.getAxis(2)), toCenter)
		);
}
...
unsigned CollisionDetector::boxAndBox(const CollisionBox &one, const CollisionBox &two, CollisionData *data)
{
	...
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(0), two.getAxis(0)), 6);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(0), two.getAxis(1)), 7);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(0), two.getAxis(2)), 8);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(1), two.getAxis(0)), 9);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(1), two.getAxis(1)), 10);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(1), two.getAxis(2)), 11);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(2), two.getAxis(0)), 12);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(2), two.getAxis(1)), 13);
	CHECK_OVERLAP(Vector3::crossProduct(one.getAxis(2), two.getAxis(2)), 14);
	...
	else
	{
		...
		Vector3 axis = Vector3::crossProduct(oneAxis, twoAxis);
		axis.normalize();
